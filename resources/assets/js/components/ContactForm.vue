		...
		<form action="post" class="form--contacts" @submit.prevent="sendForm" v-if="form.show">
			...
			<fieldset>
				...
				<div class="form-group" :class="{'invalid' : errors.has('message')}">
					<textarea id="your-message" rows="4" class="form-control" name="message" v-model="form.message" :class="{'non-empty' : form.message.length}" autocomplete="off" v-validate="'required'"></textarea>
